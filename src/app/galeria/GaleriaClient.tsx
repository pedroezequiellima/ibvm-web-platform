...
const categorias = ['Todos', 'Cultos', 'Missões', 'Jovens', 'Eventos', 'Historia'] as const;

interface GaleriaClientProps {
  dados: GaleriaItem[];
}

export default function GaleriaClient({ dados }: GaleriaClientProps) {
  const [filtroAtivo, setFiltroAtivo] = useState('Todos');

  const midiasFiltradas = dados
            ...
            <button
              ...
              className={`px-6 py-2.5 rounded-full text-sm font-medium transition-all duration-300 ${
                filtroAtivo === categoria
                  ? 'bg-[#5c4938] text-white shadow-md'
                  : 'bg-[#EBE5DB] text-[#533928] hover:bg-[#DED7CB]'
              }`}
            >
              {categoria}
            </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {midiasFiltradas.length > 0 ? (
            midiasFiltradas.map((item) => (
              <div key={item.id} className="group relative bg-white rounded-4xl overflow-hidden shadow-sm border border-[#F0E6D2] hover:shadow-lg transition-shadow duration-300">
